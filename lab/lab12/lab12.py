...
def random_rocks(grid, chance_of_rock):
    ...
    """
    new_grid = grid.copy()
    func = lambda x, y: new_grid.set(x, y, "r")
    modify_grid(new_grid, func, chance_of_rock)
    return new_grid
...
def random_bubbles(grid, chance_of_bubbles):
    ...
    """
    new_grid = grid.copy()
    func = lambda x, y: new_grid.set(x, y, "b")
    modify_grid(new_grid, func, chance_of_bubbles)
    return new_grid
...
def modify_grid(grid, func, prob):
    ...
    for y in range(grid.height):
        for x in range(grid.width):
            if grid.get(x, y) is None and random.random() <= prob:
                func(x, y)
    return grid
...
def bubble_up(grid, x, y):
    ...
    """
    new_grid = grid.copy()
    bubble = new_grid.get(x, y)
    new_grid.set(x, y-1, bubble)
